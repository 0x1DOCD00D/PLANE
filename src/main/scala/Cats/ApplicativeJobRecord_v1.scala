...
/*
* don’t make the case class JobRecord itself an Applicative (it’s a concrete value type, kind *).
* Instead use Applicative instances for effect/container types like Option, ValidatedNel, IO, etc.
* to build JobRecord from F[String] and F[Int]. That gives you generic, composable code that
