...
class InsertionSortTest extends org.scalatest.funsuite.AnyFunSuite {
  ...
  test("sort a list of many integers, no duplicates") {
    assert(ClassicMergeSort(List(5, 2, 4, 6, 1, 3)) === List(1, 2, 3, 4, 5, 6))
  }

  test("sort a list of two integers, no duplicates") {
    assert(ClassicMergeSort(List(5, 2)) === List(2, 5))
  }

  test("sort a list of one integer") {
    assert(ClassicMergeSort(List(5)) === List(5))
  }

  test("sort an empty list") {
    assert(ClassicMergeSort(List()) === List())
  }

  test("sort a list of string") {
    assert(ClassicMergeSort(List("Mark", "John", "Jeff", "6Pack", "Andrew", "Vic")) === List("6Pack", "Andrew", "Jeff", "John", "Mark", "Vic"))
  }
